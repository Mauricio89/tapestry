...
public class InsertarAlumnos { 
    ...
    void onValueChangedFromProvincia(Provincias provincia) {
        ...
        if (request.isXHR()) {
            ajaxResponseRenderer.addRender(provinciaZone);
        }
    }

    Class<ListaAlumnos> onSuccess() { 
         daoalumno.insertarAlumnos(alumno.getCedula(),alumno.getNombres(),alumno.getFnac(),alumno.getLnac(),alumno.getLibretam(),
